...
class Scraper(Crawler):
    ...
    async def _scraping(cls, page: Page, client: AsyncClient, search: str, channel_id: int, strings: list = [], is_remote: bool = True):
        ...
        await page.goto(url)
        await cls.__is_scrollable(cls, page)
        ...
        for i, job in enumerate(jobs):            
            ...
            payload['description'] = ''
            cls.urls.append(url)
            if requirements: cls.data.append(payload)
    ...
    async def __is_scrollable(cls, page: Page):
        ...
        if cls.scroll_position != scroll_position:
            ...
            return await cls.__is_scrollable(cls, page)
